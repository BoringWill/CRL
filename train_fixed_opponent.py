...
config = {
    ...
    "p1_path": "模型集_opponent/train_20260125-013011/evolution_v4.pth",
    "p2_path": "模型集_opponent/train_20260125-013011/evolution_v4.pth",
...
def train():
    ...
    while global_step < config["total_timesteps"]:
            ...
            for i in range(config["num_envs"]):
                ...
                if term[i] or trunc[i]:
                    ...
                    if current_opp_indices[i] != -1 and is_agent_win:
                        ...
                        probs = np.exp(qs - np.max(qs)) / np.sum(np.exp(qs - np.max(qs)))
                        q_scores[current_opp_indices[i]] -= config["openai_eta"] / (
                                    len(opponent_pool_paths) * probs[current_opp_indices[i]])

                    if len(opponent_pool_paths) > 0 and random.random() < config["historical_ratio"]:
                        qs = np.array(q_scores)
                        probs = np.exp(qs - np.max(qs)) / np.sum(np.exp(qs - np.max(qs)))
                        idx = np.random.choice(len(opponent_pool_paths), p=probs)
                        path, current_opp_indices[i] = opponent_pool_paths[idx], idx
                    else:
                        path, current_opp_indices[i] = opponent_model_path, -1

                    opponents[i].load_state_dict(torch.load(path, map_location=config["device"])[
                                                     "model_state_dict"] if "model_state_dict" in torch.load(path,
                                                                                                             map_location=
                                                                                                             config[
                                                                                                                 "device"]) else torch.load(
        ...
        print(
            f"步数: {global_step:7d} | 当前周期局数: {total_games:4d} | 胜率: {total_agent_win_rate:.2%}{q_info} | 对手: {os.path.basename(current_opp_paths[0])}")
